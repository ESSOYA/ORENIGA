...
// Récupérer la filière de l'étudiant depuis la table 'etudiant'
$filiere = null;
if ($etudiant_id) {
    $stmt = $conn->prepare("SELECT filiere FROM etudiant WHERE etu_id = ?");
    $stmt->bind_param("i", $etudiant_id);
    $stmt->execute();
    $stmt->bind_result($filiere);
    $stmt->fetch();
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($documents_exist) {
        $stmt = $conn->prepare("UPDATE documents_inptic SET fichier_bac=?, fichier_notes=?, fichier_naissance=?, fichier_identite=?, photos_identite=?, telephone=?, filiere=? WHERE etu_id=?");
        $stmt->bind_param("sssssssi", $fichier_bac, $fichier_notes, $fichier_naissance, $fichier_identite, $photos_identite, $telephone, $filiere, $etudiant_id);
    } else {
        $stmt = $conn->prepare("INSERT INTO documents_inptic (fichier_bac, fichier_notes, fichier_naissance, fichier_identite, photos_identite, telephone, filiere, etu_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssi", $fichier_bac, $fichier_notes, $fichier_naissance, $fichier_identite, $photos_identite, $telephone, $filiere, $etudiant_id);
    }
...
// Requête pour obtenir le statut des documents des étudiants
$query = "
SELECT 
    d.id AS document_id,
...
// Lier les résultats aux variables PHP (13 variables pour 13 colonnes sélectionnées)
$stmt->bind_result(
    $document_id,
    $d_id,
    $document_date,
...
// Stocker les résultats dans un tableau pour affichage
$students = [];
while ($stmt->fetch()) {
...
<div class="container py-5">
    ...
    <?php 
    // On vérifie que les documents sont bien chargés pour l'étudiant connecté
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <?php if (isset($students[$etudiant_id])): ?>
    <?php $student = $students[$etudiant_id]; ?>
    <?php foreach ($student['documents'] as $doc): ?>
     <?php echo ucfirst($doc['document_status']); ?>
    <?php endforeach; ?>
<?php else: ?>
    <p>Aucun document trouvé pour cet étudiant.YANN</p>
<?php endif; ?>
...
<?php endif; ?>
                    ...
                    <div class="card-body">
                        ...
                        <form id="upload-form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="tel" class="form-control" id="phone" name="telephone" placeholder="Format: 066 81 35 42 " value="<?php echo htmlspecialchars($documents['telephone'] ?? ''); ?>">
                            </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row g-4">
            <?php 
                $document_types = [
                    ...
                    'fichier_bac' => 'Attestation de réussite au BAC',
                    'fichier_notes' => 'Relevé de notes du BAC',
                    'fichier_identite' => "Pièce d'identité",
                    'photos_identite' => 'Demi carte photo sur fond blanc'
                ];

                foreach ($document_types as $field_name => $field_label): 
                    $is_image = ($field_name === 'photos_identite');
                    $current_file = $documents[$field_name] ?? '';
            ?>
            <div class="col-md-4">
                <div class="card document-card">
                    <div class="card-body text-center">
                        <i class="<?php echo $is_image ? 'fas fa-image' : 'fas fa-file-pdf'; ?> upload-icon mb-3"></i>
                        <h5><?php echo $field_label; ?></h5>
                        <input type="file" class="form-control mb-2" accept="<?php echo $is_image ? '.jpg' : '.pdf'; ?>" name="<?php echo $field_name; ?>">
                        <small class="text-muted d-block mb-2">Format <?php echo $is_image ? 'image' : 'PDF'; ?> uniquement</small>
                        <?php if (!empty($current_file)): ?>
                            <span class="text-info">Fichier actuel : <?php echo basename($current_file); ?></span>
    ...
    <?php endforeach; ?>
<?php else: ?>
    <p>Aucun document trouvé pour cet étudiant.</p>
<?php endif; ?>
