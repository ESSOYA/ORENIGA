...
if (isset($_SESSION['user'])) {
    $user_Id = $_SESSION['user']['admin_id']; // Id de l'utilisateur connecté

    // Récupérer le chemin de l'image depuis la base de données
    $sql = "SELECT Photo_profil FROM administrateur WHERE admin_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $updateSql = "UPDATE administrateur SET Photo_profil = NULL WHERE admin_id = ?";
        ...
        if ($updateStmt->execute()) {
            ...
            window.location.href = '/oreniga/page/administrateur.php';
          </script>";
        } else {
