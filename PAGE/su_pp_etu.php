...
if (isset($_SESSION['user'])) {
    $user_Id = $_SESSION['user']['etu_id']; // Id de l'utilisateur connecté

    // Récupérer le chemin de l'image depuis la base de données
    $sql = "SELECT Photo_profil FROM etudiant WHERE etu_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $updateSql = "UPDATE etudiant SET Photo_profil = NULL WHERE etu_id = ?";
        ...
        if ($updateStmt->execute()) {
            ...
            window.location.href = '/oreniga/page/etudiant.php';
          </script>";
        } else {
