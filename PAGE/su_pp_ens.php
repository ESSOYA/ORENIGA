...
if (isset($_SESSION['user'])) {
    $user_Id = $_SESSION['user']['ens_id']; // Id de l'utilisateur connecté

    // Récupérer le chemin de l'image depuis la base de données
    $sql = "SELECT Photo_profil FROM enseignant WHERE ens_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $updateSql = "UPDATE enseignant SET Photo_profil = NULL WHERE ens_id = ?";
        ...
        if ($updateStmt->execute()) {
            ...
            window.location.href = '/oreniga/page/enseignant.php';
          </script>";
        } else {
