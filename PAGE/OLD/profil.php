...
<script>
    document.getElementById('subscribe-btn').addEventListener('click', function() {
        // Change le texte du bouton et envoie une requête AJAX
        let action = this.textContent.trim() === "S'abonner" ? "subscribe" : "unsubscribe";
        ...
        fetch('abonnement.php', {
            ...
            body: `profile_id=<?php echo $profile_id; ?>&action=${action}`
        })
        .then(response => response.text())
        .then(data => {
            console.log(data); // Voir la réponse pour déboguer
            // Met à jour le texte du bouton en fonction de la réponse
            if (data === "abonné") {
                document.getElementById('subscribe-btn').textContent = "Se désabonner";
            } else if (data === "désabonné") {
                document.getElementById('subscribe-btn').textContent = "S'abonner";
            }
        })
    ...
    <div class="profile-stats">
        <div>
            ...
            <div>
                <span><?php echo $abonne_count; ?></span>
                Abonnés
            </div>
