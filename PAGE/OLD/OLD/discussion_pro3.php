...
$user_id = $_SESSION['user']['ens_id']; // ID de l'utilisateur entreprise
$user_type = $_SESSION['user']['ens_type']; // Assurez-vous que le type d'utilisateur est 'entreprise'
$type = 'enseignant';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($message || $file_path) {
        $sql = "INSERT INTO messages_pro (sender_id, sender_type, message, file_path) VALUES (?, ?, ?, ?)";
...
if (isset($_POST['like']) || isset($_POST['dislike'])) {
    ...
    if (isset($_POST['like'])) {
        $sql = "UPDATE messages_pro SET likes = likes + 1 WHERE id = ?";
    } else {
        $sql = "UPDATE messages_pro SET dislikes = dislikes + 1 WHERE id = ?";
    }
...
if (isset($_POST['delete'])) {
    ...
    $sql = "SELECT sender_id FROM messages_pro WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $message_id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        if ($message['sender_id'] == $user_id) {
            ...
            $sql = "DELETE FROM messages_pro WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $message_id);
            $stmt->execute();
        } else {
...
$sql = "SELECT messages_pro.id, messages_pro.message, messages_pro.created_at, messages_pro.sender_id, messages_pro.sender_type, messages_pro.likes, messages_pro.dislikes, messages_pro.file_path,
            CASE 
                WHEN messages_pro.sender_type = 'etudiant' THEN (SELECT CONCAT(Nom_famille, ' ', prénom) FROM etudiants WHERE etu_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'enseignant' THEN (SELECT CONCAT(Nom_famille, ' ', prénom) FROM enseignants WHERE ens_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'entreprise' THEN (SELECT username FROM entreprises WHERE entre_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'administrateur' THEN (SELECT CONCAT(Nom_famille, ' ', prénom) FROM administrateur WHERE admin_id = messages_pro.sender_id)
            END AS sender_name,
            CASE 
                WHEN messages_pro.sender_type = 'etudiant' THEN (SELECT photo_profil FROM etudiants WHERE etu_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'enseignant' THEN (SELECT photo_profil FROM enseignants WHERE ens_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'entreprise' THEN (SELECT photo_profil FROM entreprises WHERE entre_id = messages_pro.sender_id)
                WHEN messages_pro.sender_type = 'administrateur' THEN (SELECT photo_profil FROM administrateur WHERE admin_id = messages_pro.sender_id)
            END AS sender_photo,
            messages_pro.sender_type
        FROM messages_pro
        ORDER BY messages_pro.created_at DESC";
...
<body>
    <div class="container">
        <h1>Discussions professionelles</h1>


        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="message">
            <div class="profil" ><img src="<?php echo htmlspecialchars($row['sender_photo']); ?>" alt="Photo de profil"></div>
                <div class="message-content">
                    ...
                    <form action="discussion_pro3.php" method="post">
        ...
        <a href="ens.php">Retour à mon profil</a> | <a href="logout.php">Se déconnecter</a>
    </div>
