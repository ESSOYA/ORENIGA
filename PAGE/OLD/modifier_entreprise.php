...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM entreprises WHERE entre_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['photo_profil']) && $_FILES['photo_profil']['error'] === UPLOAD_ERR_OK) {
        $targetDir = "uploads/"; // Répertoire pour stocker les photos
    ...
    $updateSql = "UPDATE entreprises SET prénom=?, Nom_famille=?, username=?, email=?, téléphone=?, DATE_NAISSANCE=?, genre=?, entre_type=?, mot_de_passe=?, photo_profil=? WHERE entre_id=?";
    $updateStmt = $conn->prepare($updateSql);
    $updateStmt->bind_param("ssssssssssi", $prenom, $famille, $username, $email, $phone, $dob, $gender, $role, $password, $photoProfil, $userId);
...
<img src="IMAGE/ORENIGA_FULL.png" alt="ORENIGA" class="floating-logo">
    ...
    <div class="input-group">
        <input type="text" id="prenom" name="prenom" value="<?php echo htmlspecialchars($user['prénom']); ?>" placeholder="Prénom" required>
        <input type="text" id="famille" name="famille" value="<?php echo htmlspecialchars($user['Nom_famille']); ?>" placeholder="Nom de famille" required>
    </div>
    <input type="text" id="username" name="username" value="<?php echo htmlspecialchars($user['username']); ?>" placeholder="Nom d'utilisateur pour vous connecter après" required>
    <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" placeholder="Adresse Email" required>
    <input type="tel" id="phone" name="phone" value="<?php echo htmlspecialchars($user['téléphone']); ?>" placeholder="Téléphone" required>
    <input type="date" id="dob" name="dob" value="<?php echo htmlspecialchars($user['DATE_NAISSANCE']); ?>" placeholder="Date de Naissance" required>
    <div class="input-group">
        <select id="gender" name="gender" required>
            <option value="">GENRE</option>
            <option value="Male" <?php echo ($user['genre'] === 'Male') ? 'selected' : ''; ?>>HOMME</option>
            <option value="Female" <?php echo ($user['genre'] === 'Female') ? 'selected' : ''; ?>>FEMME</option>
            <option value="Other" <?php echo ($user['genre'] === 'Other') ? 'selected' : ''; ?>>AUTRE</option>
        </select>
        <select id="role" name="role" >
            
            <option value="ENTREPRISE" <?php echo ($user['entre_type'] === 'ENTREPRISE') ? 'selected' : ''; ?>>ENTREPRISE</option>
        </select>
    </div>
