...
$user_id = $_SESSION['user']['etu_id']; // Assurez-vous que l'ID de l'utilisateur est stocké ici
$sql = "SELECT Nom_famille, prénom,username,DATE_NAISSANCE, email, photo_profil, mot_de_passe,etu_type FROM etudiants WHERE etu_id = ?"; // Assurez-vous que 'user_id' correspond au champ de votre table
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['photo_profil'])) {
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["photo_profil"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE etudiants SET photo_profil = ? WHERE etu_id = ?"; // Assurez-vous que 'user_id' correspond au champ de votre table
            $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $BIRTH = $conn->real_escape_string($_POST['BIRTH']);
        ...
        $sql = "UPDATE etudiants SET DATE_NAISSANCE = ? WHERE etu_id = ?"; // Assurez-vous que 'user_id' correspond au champ de votre table
        $stmt = $conn->prepare($sql);
...
<header>
      <div class="logo">
      <a href="#profil"><img src="<?php echo htmlspecialchars($photo_profil); ?>" width="75" alt="Description de l'image" id="image"></a>
        <a href="#profil1">
  ...
  img.addEventListener('error', function() {
    img.src = "inco.jpg"; // Définit l'image alternative
    img.alt = "Image alternative"; // Change aussi la description si besoin
  });
</script>

        
          <span><strong><?php echo htmlspecialchars($nom); ?></strong></span
          > <span><strong> <?php echo htmlspecialchars($prenom); ?> </strong></span> </a
      ...
      <ul class="menu">
        ...
        <li>
          <a href="dash.php"><strong>Forums</strong></a>
        </li>
        <li>
          <a href=""><strong>Enseignants</strong></a>
        </li>
        <li>
    ...
    <div id="welcome-dialog">Bienvenue sur notre site !</div>

    <script>
        // Afficher la boîte de dialogue
        document.addEventListener('DOMContentLoaded', function() {
            const welcomeDialog = document.getElementById('welcome-dialog');
            welcomeDialog.style.display = 'block'; // Afficher la boîte

            

   
        // Masquer la boîte après 3 secondes (3000 millisecondes)
            setTimeout(function() {
                welcomeDialog.style.display = 
                welcomeDia'none';}, 3000);});
    </script>
        
        <div class="bienvenu" >
        <h1>Mon Profil ETUDIANT</h1>
            <H2>bienvenu</H2>
            <strong>BONJOUR </strong> <?php echo htmlspecialchars($prenom); ?>😎😎 <strong>ça va bien ? vous venez vous connecté aujourd'hui hum c'est super</strong><br><br>
