...
$user_id = $_SESSION['user']['ens_id']; // Assurez-vous que l'ID de l'utilisateur est stocké ici
$sql = "SELECT Nom_famille, prénom, email, photo_profil FROM enseignants WHERE ens_id = ?"; // Assurez-vous que 'user_id' correspond au champ de votre table
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['photo_profil'])) {
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["photo_profil"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE enseignants SET photo_profil = ? WHERE ens_id = ?"; // Assurez-vous que 'user_id' correspond au champ de votre table
            $stmt = $conn->prepare($sql);
...
<body>
    <div class="container">
        ...
        <a href="discussion3.php">Accéder à la discussion</a>

        <a href="logout.php">Se déconnecter</a>
        <a href="/ORENIGA/INDEX.HTML">REVENIR AU MENU PRINCIPAL</a>
    </div>
</body>
</html>
